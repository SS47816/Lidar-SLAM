...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    const Eigen::Vector2d mu = std::accumulate(nearPoints.begin(), nearPoints.end(), Eigen::Vector2d(0, 0)) / nearPoints.size();

    // Construct the matrix for all the normalized points
    std::for_each(nearPoints.begin(), nearPoints.end(), [mu](Eigen::Vector2d vec) { vec -= mu; });
    ...
    const std::pair<Eigen::Vector2d, Eigen::Vector2d> evectors{es.eigenvectors().col(0).real(), es.eigenvectors().col(1).real()};
    normal = evalues[0] < evalues[1] ? evectors.first : evectors.second;
